...
<table class="table">
	<tr>
		...
		<td>
			<table class="table table-bordered">
				...
				<tr>
					<td><input type="text" name="first_name" class="required text2"
						value="<?php echo @$row->first_name;?>" /></td>
					<td><input type="text" name="middle_name" class="text2"
						value="<?php echo @$row->middle_name;?>" /></td>
					<td><input type="text" name="last_name" class="text2"
	...
	<tr>
		...
		<td><?php echo HTMLUtils::renderSelect(null, 'gender', MVCHelper::lookup('gender', '__cgaf'), @$row->gender, false) ?>
		</td>
	</tr>
	<tr>
		<td class="title"><?php echo __('religion', 'Religion') ?></td>
		<td><?php echo HTMLUtils::renderSelect(null, 'religion', MVCHelper::lookup('religion', '__cgaf'), @$row->religion, false) ?>
		</td>
	</tr>
	<tr>
		<td class="title"><?php echo __('birthdate', 'Birth Date') ?></td>
		<td><label> <input type="text" id="birth_date" name="birth_date" data-input="date"
				class="date"
				minYear="<?php echo date('Y') - (int)$this->getAppOwner()->getConfig("user.minage", 15) ?>"
				value="<?php echo @$row->birthdate ?>" />&nbsp; <em><strong>format </strong>
					<?php echo $dateFormat?> </em>
		</label>
		</td>
	</tr>
	<tr>
		<td class="title"><?php echo __('address', 'Address') ?></td>
		<td><textarea name="address" class="required text"></textarea>
		</td>
	</tr>
	<?php
}
if ($app->getConfig('user.register.captcha', true)) {
  ?>
	<tr>
		<td class="title"><?php echo __('verificationcode', 'Verification Code') ?>
		</td>
		<td><?php echo HTMLUtils::renderCaptcha(null, null, false) ?></td>
	</tr>
	<?php } ?>
	<tr>
		<td colspan="2" align="right"><?php echo sprintf(__('user.register.agree'), '<a href="' . BASE_URL . 'info/view/?type=register" class="agree" rel="__overlay">', '</a>') ?>
		</td>
	</tr>
</table>
