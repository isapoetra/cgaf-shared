...
class MessageCollection  implements \Iterator {
	...
	public function current () {
		return $this->_d[$this->_keys[$this->_pos]];
	}
	public function key () {
	...
	public function valid (  ) {
		return isset($this->_keys[$this->_pos]) && isset($this->_d[$this->_keys[$this->_pos]]);
	}
}
//TODO Optimize and split based on received
class UserMessages extends \BaseObject{
	private $_storePath;
	private $_messageInfo;
	private $_appOwner;
	function __construct($appOwner,$uid=null) {
		$this->_appOwner= $appOwner;
		$this->_storePath = \CGAF::getUserStorage($uid);
		\Utils::makeDir($this->_storePath);
		$fInfo=$this->_storePath.'messages.bin';
		$this->_messageInfo = array('unread'=>0,'count'=>0);
		if (is_file($fInfo)) {
			$this->_messageInfo = unserialize(file_get_contents($fInfo));
		}
	}
	function __destruct() {
		if ($this->_messageInfo) {
			file_put_contents($this->_storePath.'messages.bin', serialize($this->_messageInfo));
		}
	}
	function del($id) {
		if (!isset($this->_messageInfo['messages'][$id])) {
			return;
		}
		if ($this->_messageInfo['messages'][$id]['flags'] ==='trash') {
			@unlink($this->_storePath.'messages/'.$id.'.msg');
			unset($this->_messageInfo['messages'][$id]);
		}else{
			$this->_messageInfo['messages'][$id]['flags'] = 'trash';
		}
	}
	function getMessageByFlag($flag) {
		$muser = $this->_appOwner->getModel('person');
		$retval = array();
		foreach($this->_messageInfo['messages'] as $k=>$v) {
			if ($v['flags'] === $flag) {
				if (!@$this->_messageInfo['messages'][$k]['parsed']) {
					$this->_messageInfo['messages'][$k] = $this->parseMsg($v);
					$this->_messageInfo['messages'][$k]['parsed'] =true;
				}
				$retval[$k] = $this->_messageInfo['messages'][$k];
			}
		}
		return $retval;
	}
	...
	private function parseMsg($msg) {
		$muser = $this->_appOwner->getModel('person');
		if (isset($msg['from'])) {
			$p =$muser->getPersonByUser($msg['from']);
			if ($p) {
				$msg['sender'] = $p->fullname;
				$msg['senderemail'] =  $p->email;
			}
		}
		return $msg;
	}
	function readMessage($id) {
		...
		$msg = $this->parseMsg($this->_messageInfo['messages'][$id]);
		if (!@$msg['read']) {
			$this->_messageInfo['unread']--;
			$this->_messageInfo['messages'][$id]['read'] =true;
		}
		$fmsg =$this->_storePath.'messages/'.$id.'.msg';
		if (is_file($fmsg)) {
			$msg['contents'] = file_get_contents($fmsg);
		}else{
			$msg['contents']='';
		}
	...
	function __get($key) {
		switch (strtolower($key)) {
			...
			case 'count':
			case 'read':
				return $this->_messageInfo[$key];
		}
		return new MessageCollection($this->getMessageByFlag($key));;
	}
...
class UserController extends Controller {
	...
	function dashboard() {
		...
		$contents =null;
		if ($v=\Request::get('__v')) {
			$v = explode('.', $v);
			if (count($v)==1) {
				$v[1] = 'index';
			}
			$c = $this->getAppOwner()->getController($v[0]);
			if ($c && $c->isAllow($v[1])) {
				$params = array();
				$c->initAction($v[1], $params);
				$contents= $c->{$v[1]}();
			}
		}
	...
	function renderMessageByFlag($flag) {
		$info = $this->getMyMessages();
		return parent::renderView('messages/byflag',array(
				'flag'=>$flag,
				'messages'=>$info,
				'rows'=>$info->{$flag}));
	}
	public function messages() {
		$rurl = explode('/',$_REQUEST['__url']);
		array_shift($rurl);
		array_shift($rurl);
		$info = $this->getMyMessages();
		...
		if (count($rurl)) {
			switch (strtolower($rurl[0])) {
				case 'read':
					return parent::renderView('messages/read',array(
					'row'=>$info->readMessage($rurl[1])));
				case 'inbox':
				case 'outbox':
				case 'trash':
					return $this->renderMessageByFlag($rurl[0]);
				case '':
					break;
				default:
					ppd($rurl);
		...
		return parent::renderView(__FUNCTION__,array('messages'=>$info,'flag'=>'inbox'));
	}
	public function getMyMessages() {
		if ($this->_myMessageInfo ===null) {
			$this->_myMessageInfo = new UserMessages($this->getAppOwner(), ACLHelper::getUserId());
		}
		return $this->_myMessageInfo;
	}
