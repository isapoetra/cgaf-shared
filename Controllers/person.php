...
class Person extends Controller {
    ...
    switch ($access) {
      ...
      case ACLHelper::ACCESS_UPDATE:
        if ($this->getAppOwner()->isAuthentificated()) {
          ...
          ->loadObject();
          if ($p->pt >= (int) \CGAF::getConfig('person.maximum',1)) {
  ...
  function aed($row=null,$action='aed',$args=null) {
    ...
    if (!$id) {
      ...
      ->where('person_owner='.ACLHelper::getUserId())
      ->loadObject();
      if ($p->pt > (int) $this->getAppOwner()->getConfig('app.person.maximum',1)) {
        if (!ACLHelper::isInrole(ACLHelper::ADMINS_GROUP)) {
  ...
  function app() {
    return $this->contacts($this->getAppOwner()->getConfig('app.maincontact'));
  }
  function contacts($args = null) {
    ...
    $args['rows'] = $pi->getContacts();
    return parent::render(__FUNCTION__, $args);
  }
  ...
  function detail($args = null, $return = null) {
    ...
    $actions = array();
    if ($o->person_owner === ACLHelper::getUserId()) {
      $actions[] = new Anchor(\URLHelper::add(APP_URL,'company/pick/'), 'Pick Company');
  ...
  public function myperson($configs) {
    ...
    $rows= $this->getModel()->clear()->where('person_owner=' . ACLHelper::getUserId())->loadObjects();
